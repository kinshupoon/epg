...
def get_desc(url_part):  # 获取节目介绍,总是出问题，暂不使用
  ...
  try:
    ...
    if soup.select('div.section'):
      ...
      desc = soup.select('div.d_s_info')[0].text + soup.select(
          'div.desc_col')[0].text
...
def get_epgs_tvmao2(channel, channel_id, dt, func_arg):
  ...
  if len(id_split) == 2:
    ...
    id = channel_id
  url = "https://lighttv.tvmao.com/qa/qachannelschedule?epgCode=%s&op=getProgramByChnid&epgName=&isNew=on&day=%s" % (
      id, need_weekday)
  try:
    ...
    for data in datas:
      ...
      starttime = datetime.datetime.combine(
          dt, datetime.time(int(starttime_str[:2]), int(
              starttime_str[-2:]))) - datetime.timedelta(hours=8)  #服务器不能设北京时区的，已减8小时
      epg = {
          'channel_id': channel.id,
          'starttime': starttime,
          'endtime': None,
